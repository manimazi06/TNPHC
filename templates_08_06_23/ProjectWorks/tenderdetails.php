    ...
    <div class="card card-topline-aqua">
         ...
         <div id ="project" style="display:none;"> </div>   
		  ...
		  <?php if ($project_tender_statusescount  > 0) { ?>
		   <div class="card-body">
			   ...
			   <?php } ?> 
		  <?php  if($tender_statuses_last['tender_status_id'] < 4){ ?>
          <div id="tenderstatus">
			 <div class="card-body">
        ...
        <?php //if ($projectWorkSubdetail['tender_detail_flag'] == 0) { ?>
            <div id="addpage" style="display:none;">
			 <div class="card-body"  <?php  //if($tender_count == 1){ ?>  <?php //} ?>>
                ...
                <tbody>
                    ...
                    foreach ($tenders as $projectTenderDetail) : ?>
                        <tr class="text-center">
							...
							<?php  if($projectTenderDetail['tender_type_id'] == 1){ 
							    $tender_ID =  'Etender-'.$projectTenderDetail['etenderID']; 
								}else if($projectTenderDetail['tender_type_id'] == 2){ 
								 $tender_ID = 'Boxtender-'.$projectTenderDetail['tender_no'];  
								} ?>
                            ...
                            <?php //if ($projectWorkSubdetail['tender_detail_flag'] == 0) { ?>
                                <td class="text-center">
                                    <span style="overflow: visible; position: relative; width: 177px;">
									   ...
									   <?php } ?>
										<?php echo $this->Html->link(__('<i class="fa fa-pencil"></i>Contractor Details'), ['action' => 'addcontractor', $id, $work_id, $projectTenderDetail['id']], ['escape' => false, 'class' => 'btn btn-outline-success btn-sm']); ?><br><br>
										<?php echo $this->Html->link(__('<i class="fa fa-trash"></i>Cancel Tender'), ['action' => 'projecttendercancel',$id,$work_id,$projectTenderDetail['id']], ['confirm' => __('Are you sure you want to Cancel Tender - {0}?', $tender_ID), 'class' => 'btn btn-outline-danger btn-sm', 'escape' => false]); ?>
...
<?php  }*/ ?>
<?php if($contractor_detail_count == 1 && $tender_count == 1 && $projectWorkSubdetail['tender_detail_flag'] == 0) {  ?>
        <div class="form-group" style="padding-top: 10px;">
            <div class="offset-md-5 col-md-10">
                <?php echo $this->Form->control('completed_flag', ['label' => false, 'error' => false, 'type' => 'hidden']) ?>
				<?php if($projectWorkSubdetail['project_work_status_id'] >= 9){ ?>
				<?php echo $this->Form->control('edit_flag', ['label' => false, 'error' => false, 'type' => 'hidden','value'=>1]) ?>				
				<?php } ?>
...
<script>
    $(document).ready(function() {
        ...
        $('#add').click(function() {
            ...
            $.ajax({
                async: true,
                dataType: "html",
                url: '<?php echo $this->Url->webroot ?>/ProjectTenderDetails/ajaxprojecttender/' +
                    j,
  ...
  $(document).ready(function() {
	...
	<?php /*if(($tender_statuses_last['tender_status_id'] < 4) || ($project_tender_statusescount == '')){ ?>
	$('#addpage').hide();
	<?php }else{ ?>
	$('#addpage').show();	
	<?php }*/ ?>	
