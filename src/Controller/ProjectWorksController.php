...
{   
   public function dashboard()
	...
	where project.is_active=1 $condition";
			...
			$query1                 =  "SELECT count(project.id) AS approved
										from project_work_subdetails as project 
										 where  project.is_active=1 and project.project_work_status_id>=3 and project.work_type = 1 $condition";
			...
			$query2                =  "SELECT 
								   count(project.id)AS sanctioned
									from project_work_subdetails as project 
								   where project.is_active=1 and project.project_work_status_id>=5 and project.work_type = 1 $condition";
			// echo "<pre>";
			// print_r($query2);
			// exit();
			...
			$query3                =  "SELECT 
									...
									from project_work_subdetails as project 
								   where project.is_active=1 and project.project_work_status_id >=11 $condition";
			// echo "<pre>";
			// print_r($query3);
			// exit();
			...
			$query5                =  "SELECT 
										count(project.id) AS raised,sum(project.amount) as uc_raised_amount
									from utilization_certificates as project 
								   where project.is_active=1";
			$uc_raised    = $connection->execute($query5)->fetchAll('assoc');
			
			$query6                =  "SELECT 
										count(project.id) AS received,sum(project.amount) as uc_received_amount
									from uc_fund_sanctioned_details as project 
								  ";
			$uc_amount_received    = $connection->execute($query6)->fetchAll('assoc');
			
			
			$balance_uc_amount = $uc_raised[0]['uc_raised_amount'] - $uc_amount_received[0]['uc_received_amount'];
			//print_r($balance_uc_amount); exit();
			
			$query7                =  "SELECT 
										o.opening_balance AS amount
									from opening_balance_details as o 
								  ";
			$pd_account    = $connection->execute($query7)->fetchAll('assoc');
			...
			foreach ($financialYears as $key => $year) {
				$query_yearwise = "select d.name as depart_name,sum(as_sac.sanctioned_amount) as sanctioned_amount
					...
					where project.is_active = 1 and fs.id = " . $key . " $condition
			...
			foreach ($departmentsname as $key => $value) {
				...
				sum(CASE WHEN project.project_work_status_id>=3  THEN 1 ELSE '0' END)AS approved,
				sum(CASE WHEN project.project_work_status_id>=5 THEN 1 ELSE '0' END)AS sanctioned,
				sum(CASE WHEN project.project_work_status_id>=11 THEN 1 ELSE '0' END)AS progress,
				sum(CASE WHEN project.project_work_status_id=19 THEN 1 ELSE '0' END)AS completed
					  ...
					  where  project.work_type = 1 and  project.is_active=1  and projectwork.department_id =$key $condition ";
				// echo "<pre>";
				// print_r($query);
				// exit();
			...
			$department_details_repair = array();
			foreach ($departmentsname as $key1 => $value1) {

				$query1 = "Select count(project.id) as projectcount,
				sum(CASE WHEN project.project_work_status_id>=1  THEN 1 ELSE '0' END)AS proposed,
				...
				sum(CASE WHEN project.project_work_status_id=19 THEN 1 ELSE '0' END)AS completed
					  ...
					  from project_work_subdetails as project
					  LEFT JOIN project_works as projectwork on  projectwork.id = project.project_work_id
					  where  project.work_type = 2 and  project.is_active=1  and projectwork.department_id =$key1 $condition ";
				// echo "<pre>";
				// print_r($query);
				// exit();
				$Totalcount1      = $connection->execute($query1)->fetchAll('assoc');
				$department_details_repair[$key1]['department_name'] = $value1;
				$department_details_repair[$key1]['projectcount']    = $Totalcount1[0]['projectcount'];
				$department_details_repair[$key1]['proposed']        = ($Totalcount1[0]['proposed'] != '') ? $Totalcount1[0]['proposed'] : 0;
				$department_details_repair[$key1]['approved']        = ($Totalcount1[0]['approved'] != '') ? $Totalcount1[0]['approved'] : 0;
				$department_details_repair[$key1]['sanctioned']      = ($Totalcount1[0]['sanctioned'] != '') ? $Totalcount1[0]['sanctioned'] : 0;
				$department_details_repair[$key1]['progress']        = ($Totalcount1[0]['progress'] != '') ? $Totalcount1[0]['progress'] : 0;
				$department_details_repair[$key1]['completed']       = ($Totalcount1[0]['completed'] != '') ? $Totalcount1[0]['completed'] : 0;
			}
			...
			foreach ($divisions as $key1 => $divistionvalue) {
				...
				sum(CASE WHEN project.project_work_status_id=19 THEN 1 ELSE '0' END)AS completed
				from project_work_subdetails as project
				where  project.work_type = 1 and project.is_active=1  and project.division_id=$key1 $condition";
			...
			$divisions_details_repair = array();
			foreach ($divisions as $key2 => $divistionvalue2) {


				$query2 = "Select count(project.id) as projectcount,
				sum(CASE WHEN project.project_work_status_id>=1  THEN 1 ELSE '0' END)AS proposed,
				sum(CASE WHEN project.project_work_status_id>=2  THEN 1 ELSE '0' END)AS approved,
				sum(CASE WHEN project.project_work_status_id>=5 THEN 1 ELSE '0' END)AS sanctioned,
				sum(CASE WHEN project.project_work_status_id>=11 THEN 1 ELSE '0' END)AS progress,
				sum(CASE WHEN project.project_work_status_id=19 THEN 1 ELSE '0' END)AS completed
				from project_work_subdetails as project
				where project.work_type = 2 and project.is_active=1  and project.division_id=$key2 $condition
				";

				$Totalcount2      = $connection->execute($query2)->fetchAll('assoc');

				$divisions_details_repair[$key2]['division_name'] = $divistionvalue2;
				$divisions_details_repair[$key2]['projectcount']  = $Totalcount2[0]['projectcount'];
				$divisions_details_repair[$key2]['proposed']      = $Totalcount2[0]['proposed'];
				$divisions_details_repair[$key2]['approved']      = $Totalcount2[0]['approved'];
				$divisions_details_repair[$key2]['sanctioned']    = $Totalcount2[0]['sanctioned'];
				$divisions_details_repair[$key2]['progress']      = $Totalcount2[0]['progress'];
				$divisions_details_repair[$key2]['completed']     = $Totalcount2[0]['completed'];


			}

		$this->set(compact('balance_uc_amount','pd_account','uc_amount_received','uc_raised','notification_count','yearwise_detail', 'financialYears', 'departwise_as_details', 'progressCount', 'Totalcompletecount', 'TotalProjectCount', 'department_details', 'divisions_details', 'role_id', 'division_id', 'projectProposedCount', 'projectApprovedCount', 'projectSanctionedcount', 'projectProgresscount', 'projectCompletedcount','divisions_details_repair','department_details_repair'));
	}
	...
	public function ajaxprojectwise($val = NULL)
    {
        ...
        if ($val == 1) {
            ...
            $Cond = " AND  work_subdetails.project_work_status_id>=3 and work_subdetails.work_type = 1";
        } elseif ($val == 3) {
            $Cond = " AND  work_subdetails.project_work_status_id>=5  and work_subdetails.work_type = 1";
        } elseif ($val == 4) {
	...
	public function index()
    {
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo '<pre>'; print_r($this->request->getData()); exit();		   
			  ...
			  $query              =  "SELECT project.*,fy.name as financial_year,d.name as department_name
									 ...
									 where project.ce_approved=0 and psd.is_active = 1 and psd.old_project_work_detail_id is NULL $condition  $fin_year_cond  $dept_cond  $project_cond $dist_cond group by project.id";											 
			  ...
			  $projectWorks       = $connection->execute($query)->fetchAll('assoc'); 	
				    ...
				    $query        =  "SELECT project.*,fy.name as financial_year,d.name as department_name
										 ...
										 LEFT JOIN financial_years as fy on fy.id= project.financial_year_id 
										 LEFT JOIN project_work_subdetails as psd on psd.project_work_id = project.id
										 where project.ce_approved=0 and psd.is_active = 1 and psd.old_project_work_detail_id is NULL $condition group by project.id";											 
    ...
    {        
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo '<pre>'; print_r($this->request->getData()); exit();
			  ...
			  $dept_cond          = ($this->request->getData('department_id') != '')?" AND project.department_id = ".$this->request->getData('department_id')."":""; 				  
			  $project_cond       = ($this->request->getData('project_code')!= '')?" AND project.project_code like '%".$this->request->getData('project_code')."%'":"";
			  $div_cond          = ($this->request->getData('division_id')!= '')?" AND psd.division_id = '".$this->request->getData('division_id')."'":"";
			  $status_cond          = ($this->request->getData('status_id')!= '')?" AND psd.project_work_status_id >= '".$this->request->getData('status_id')."'":"";

	
			 /* $query              =  "SELECT project.*,fy.name as financial_year,d.name as department_name,dwt.name as work_type,dv.name as division_name
									 from project_works as project 
									 LEFT JOIN departments as d on d.id= project.department_id 
									 LEFT JOIN financial_years as fy on fy.id= project.financial_year_id 
									 LEFT JOIN project_work_subdetails as psd on psd.project_work_id = project.id
				...
				$query            =  "SELECT psd.*,fy.name as financial_year,d.name as department_name,dwt.name as work_type,dv.name as division_name,dis.name as district_name,s.name as pws,project.id as project_id
									     ...
									     LEFT JOIN project_work_statuses as s on s.id = psd.project_work_status_id
										 where project.ce_approved=1 and psd.is_active = 1 $condition  $fin_year_cond  $dept_cond  $project_cond $div_cond  $status_cond  group by psd.id";								 
			  $projectWorks       = $connection->execute($query)->fetchAll('assoc'); 
  ...
  // print_r($query);	 exit();		  
                   ...
                   $query            =  "SELECT psd.*,fy.name as financial_year,d.name as department_name,dwt.name as work_type,dv.name as division_name,dis.name as district_name,s.name as pws,project.id as project_id
									     ...
									     LEFT JOIN districts as dis on dis.id = psd.district_id
									     LEFT JOIN project_work_statuses as s on s.id = psd.project_work_status_id
										 where project.ce_approved=1 and psd.is_active = 1 $condition  group by psd.id";											 
          ...
          $work_statuses = $this->ProjectWorkStatuses->find('list')->toArray();		

	   $this->set(compact('work_statuses','divisions','departments','financialYears','projectWorks','adminsanction_count','financial_count','techsanction_count','tender_count','monitoring_count','role_id','subdetail_present','estimate_uploaded'));
    }
...
//echo '<pre>';  print_r($projectWorkSubdetails);   exit();
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {  //echo '<pre>'; print_r($this->request->getData());   exit();   
            ...
            if ($attachment != '') {
                ...
                if ($name != '' && $error == 0) {
                    $file                                      =  $name;
					//$filename                                  = trim(str_replace(" ","_", $file));
                    $array                                     =  explode('.', $file);
        ...
        $financialYears = $this->FinancialYears->find('list')->where(['FinancialYears.is_active'=>1,'FinancialYears.id >'=>14])->order(['id Desc'])->all();
		...
		if ($this->request->is(['patch', 'post', 'put'])) { 
		   ...
		   $connection = ConnectionManager::get('default');
			  $query   = "select count(project.id) as pcount
						  ...
						  LEFT join project_work_subdetails as project on project.project_work_id = p.id
						  where  project.work_type = 1 and project.old_project_work_detail_id is NULL and p.financial_year_id = ".$fin_year." and p.department_id = ".$dept_id." and p.ce_approved = 1 and project.project_work_status_id < 3
						  ";
		   ...
		   if($approved_project_count > 0){
			$approved_sub_projects  = $this->ProjectWorkSubdetails->find('all')->contain(['Districts', 'Divisions','Circles'])->where(['ProjectWorkSubdetails.project_work_id IN' => $approved_project_ids,'ProjectWorkSubdetails.project_work_status_id <'=>3,'ProjectWorkSubdetails.is_active'=>1,'ProjectWorkSubdetails.work_type'=>1,'ProjectWorkSubdetails.old_project_work_detail_id is NULL'])->toArray();      
			$approved_sub_count     = $this->ProjectWorkSubdetails->find('all')->where(['ProjectWorkSubdetails.project_work_id IN' => $approved_project_ids,'ProjectWorkSubdetails.project_work_status_id <'=>3,'ProjectWorkSubdetails.is_active'=>1,'ProjectWorkSubdetails.work_type'=>1,'ProjectWorkSubdetails.old_project_work_detail_id is NULL'])->count();      
		   }
		}  
   ...
   public function financialsanctionadd(){    
		...
		if ($this->request->is(['patch', 'post', 'put'])) { 		
			...
			if($approved_project_count > 0){
				$approved_sub_projects_count  = $this->ProjectWorkSubdetails->find('all')->contain(['Districts', 'Divisions','Circles'])->where(['ProjectWorkSubdetails.project_work_id IN' => $approved_project_ids,'ProjectWorkSubdetails.project_work_status_id'=>4,'ProjectWorkSubdetails.is_active'=>1,'ProjectWorkSubdetails.old_project_work_detail_id is NULL'])->count();      
				$approved_sub_projects        = $this->ProjectWorkSubdetails->find('all')->contain(['Districts', 'Divisions','Circles'])->where(['ProjectWorkSubdetails.project_work_id IN' => $approved_project_ids,'ProjectWorkSubdetails.project_work_status_id'=>4,'ProjectWorkSubdetails.is_active'=>1,'ProjectWorkSubdetails.old_project_work_detail_id is NULL'])->toArray();      
			}	
		}		
	    $departments    = $this->Departments->find('list')->where(['Departments.is_active'=>1])->all();
        $financialYears = $this->FinancialYears->find('list')->where(['FinancialYears.is_active'=>1])->order('id DESC')->all();		
	   ...
	   $projectWorkSubdetails       = $this->ProjectWorkSubdetails->find('all')->contain(['Divisions','Circles'])->where(['ProjectWorkSubdetails.project_work_id' => $id])->toArray();
        ...
        if ($this->request->is(['patch', 'post', 'put'])) { //echo '<pre>';  print_r($this->request->getData());   exit();			
			   foreach ($this->request->getData('project') as $key => $value) {                
				...
				$projectWorkSubdetail->submit_date             =  date('Y-m-d');
                $projectWorkSubdetail->created_by              =  $user->id;
                $projectWorkSubdetail->created_date            =  date('Y-m-d:h:i:s');				
   ...
   {	   
	 ...
	 public function tenderdetails($id = null, $work_id = null)
    {
		...
		//print_r($projectWorkSubdetailcount); exit();
		...
		if($projectWorkSubdetailcount > 0){
			$projectWork = $this->ProjectWorks->get($id, [
				'contain' => ['Departments', 'FinancialYears', 'BuildingTypes', 'ProjectStatuses', 'Districts', 'Divisions', 'SchemeTypes'],
				 ...
				 $projectWork = $this->ProjectWorks->get($id, [
				'contain' => ['Departments', 'FinancialYears', 'ProjectStatuses'],
			]);
		}		
        $administrativesanctioncount = $this->ProjectAdministrativeSanctions->find('all')->contain(['ProjectWorks'])->where(['project_work_id' => $id])->count();
        $administrativesanction      = $this->ProjectAdministrativeSanctions->find('all')->contain(['ProjectWorks'])->where(['ProjectAdministrativeSanctions.project_work_id' => $id])->first();
      ...
      foreach($tenders as $tender){
	   ...
	   $tender_count = $this->ProjectTenderDetails->find('all')->where(['ProjectTenderDetails.project_work_id' => $id, 'ProjectTenderDetails.project_work_subdetail_id' => $work_id,'ProjectTenderDetails.is_active'=>1])->count();
        ...
        if($tender_count == 0){
		   ...
		   $technical      = $this->TechnicalSanctions->find('all')->contain(['ProjectWorks'])->where(['TechnicalSanctions.project_work_id' => $id, 'TechnicalSanctions.project_work_subdetail_id' => $work_id])->first();
           if($abstract_detail['sub_total_2'] == ''){
		     $sanctioned_amount = $technical['amount'];
		   }else{
             $sanctioned_amount = $abstract_detail['sub_total_2'];
		   }  
    ...
    public function workview($id = null,$work_id = null)
    {
        ...
        $this->loadModel('ProjectwiseAbstractAdditionalDetails');
       ...
       $additional_count   = $this->ProjectwiseAbstractAdditionalDetails->find('all')->where(['ProjectwiseAbstractAdditionalDetails.project_work_id' => $id, 'ProjectwiseAbstractAdditionalDetails.project_work_subdetail_id' => $work_id,'ProjectwiseAbstractAdditionalDetails.is_active'=>1])->count();
	   $additional_details = $this->ProjectwiseAbstractAdditionalDetails->find('all')->where(['ProjectwiseAbstractAdditionalDetails.project_work_id' => $id, 'ProjectwiseAbstractAdditionalDetails.project_work_subdetail_id' => $work_id,'ProjectwiseAbstractAdditionalDetails.is_active'=>1])->toArray();


 	  $this->set(compact('abstract_detail','additional_count','additional_details','projectwiseQuartersDetailcount','projectwiseQuartersDetail_lists','project_tender_statusescount','project_tender_statuses','projectwiseTimeExtensionDetailcount','projectwiseTimeExtensionDetailists','DevelopmentWorkscount','DevelopmentWorkslists','placedtoboardcount','placedtoboarddetails','abstractcount','abstract_subdetails','completioncount','completiondetails','handovercount','handoverdetails','utilizationCertificatecount','utilizationCertificate','projectWorkSubdetailscount','detailed_approval_stages_count','detailed_approval_stages','projectTenderDetail', 'projectWork', 'tenders', 'id','administrativesanctioncount','administrativesanction','projectWorkSubdetails','financialSanctionscount','financialSanctions','work_id','contractor_detail_count','contractor_details','technicalcount','technical','fundrequests','requestcount','timelineDetailscount','timelineDetails','monitoringDetailscount','monitorings','photo_uploads','detailed_estimates','detailed_estimates_count','planningcount','planningdetail'));
    }
   ...
   public function planningpermission($id = null, $work_id = null)
    ...
    $apporved = [1 => 'Yes', 2 => 'No',3=>'Not Applicable'];
    if ($Planningcount == 0) {
       ...
       // if($insert_id != ''){
            if ($totalunit != '') {
				if(($this->request->getData('floors')!= '')){
					...
					if($this->request->getData('is_planning_approval_send') == 2 ||$this->request->getData('is_planning_approval_send') == 3){
    ...
    {        
		...
		if($type == 0){
          ...
          // $type_cond = " AND psd.detailed_estimate_approval = 0 AND psd.project_work_status_id IN (5,6,7) or (psd.project_work_status_id IN (4,5,6,7) and psd.work_type = 2)";
           $type_cond = " AND psd.detailed_estimate_approval = 0 and psd.project_work_status_id >= 4";
           //$type_cond = "";
		}else if($type == 2){	
           ...
           $title = "Quarters list";		 
           $type_cond = " AND  psd.project_work_status_id >= 2 AND project.departmentwise_work_type_id = 1  AND  psd.work_type = 1";
		}else if($type == 12){	
           ...
           $type_cond = " AND psd.project_work_status_id >= 1";  
		}			
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo '<pre>'; print_r($this->request->getData()); exit();		   
			 ...
			 /* $query              =  "SELECT psd.*,dis.name as district_name,dv.name as division_name,c.name as circle_name,ps.name as work_status,project.project_code as project_code
									 from project_work_subdetails as psd 
									 RIGHT JOIN project_works as project on project.id = psd.project_work_id
									 LEFT JOIN districts as dis on dis.id= psd.district_id 
									 LEFT JOIN divisions as dv on dv.id= psd.division_id 
...
*/									 
		  ...
		  $query            =  "SELECT psd.*,dis.name as district_name,dv.name as division_name,c.name as circle_name,ps.name as work_status,project.project_code as project_code
									     ...
									     LEFT JOIN circles as c on c.id= psd.circle_id 
									     LEFT JOIN project_work_statuses as ps on ps.id= psd.project_work_status_id 
									 where project.ce_approved=1 and psd.is_active = 1 $condition  $fin_year_cond  $dept_cond  $project_cond $district_cond $type_cond group by psd.id";
...
// print_r($query); exit();
     ...
     $detailed_approval_stages        = $this->DetailedEstimateApprovalStages->find('all')->contain(['ApprovalStatuses'])->where(['DetailedEstimateApprovalStages.project_work_id' => $id,'DetailedEstimateApprovalStages.project_work_subdetail_id'=>$work_id])->toArray();
	  ...
	  // print_r($Totalamount); exit();
	   ...
	   $tot_amount =  $Totalamount[0]['total_amount'];
	   	$technical       = $this->TechnicalSanctions->find('all')->where(['TechnicalSanctions.project_work_id' => $id, 'TechnicalSanctions.project_work_subdetail_id' => $work_id])->first();
    ...
    if ($this->request->is(['patch', 'post', 'put'])) {
		...
		if($completed_flag == 1){	
		     ...
		     if($this->request->getData('grand_total') != ''){
			        $abstractdetailTable                      = $this->getTableLocator()->get('ProjectwiseAbstractDetails');
					$abstract                                 = $abstractdetailTable->get($abstract_detail['id']); 
					$abstract->sub_total_1                    = $this->request->getData('sub_total_1');
					$abstract->gst_at_18                      = $this->request->getData('gst_at_18');
					$abstract->sub_total_2                    = $this->request->getData('sub_total_2');
					$abstract->sub_total_3                    = $this->request->getData('sub_total_3');
					$abstract->labour_welfare                 = $this->request->getData('labour_welfare');
					$abstract->contigency_petty_supervision_1 = $this->request->getData('contigency_petty_supervision_1');
					$abstract->supervision_2                  = $this->request->getData('supervision_2');
					$abstract->gst_supervision_2              = $this->request->getData('gst_supervision_2');
					$abstract->total                          = $this->request->getData('total');
					$abstract->advertisement                  = $this->request->getData('advertisement');
					$abstract->grand_total                    = $this->request->getData('grand_total');
					$abstractdetailTable->save($abstract);
			  }	

				$norms = $this->request->getData('norms');
				
			   if(count($norms) > 0){ 
					foreach($norms as $norm){					            
					 if($norm['description'] != ''){			
					 if($norm['additional_id'] != ''){			
						$additional = $this->ProjectwiseAbstractAdditionalDetails->get($norm['additional_id'], ['contain' => [],]);
						$additional->modified_by            = $user->id;
					    $additional->modified_date          = date('Y-m-d H:i:s');
	                 }else{
						$additional = $this->ProjectwiseAbstractAdditionalDetails->newEmptyEntity();
						$additional->created_by             = $user->id;
					    $additional->created_date           = date('Y-m-d H:i:s');
					 }
					 $additional->project_work_id           = $id;
					 $additional->project_work_subdetail_id = $work_id;
					 $additional->description               = $norm['description'];
					 $additional->amount                    = $norm['amount'];
					 $this->ProjectwiseAbstractAdditionalDetails->save($additional);
				  }
				  }
			  }	
	     ...
	     if ($technical['id'] != '') {
             ...
             $technicalSanction->description               = $this->request->getData('description');
             $technicalSanction->amount                    = $this->request->getData('amount');
			 $technicalSanction->gst                       = ($this->request->getData('gst_at_18'))?$this->request->getData('gst_at_18'):0;  
             $technicalSanction->created_by                = $user->id;
             $technicalSanction->created_date              = date('Y-m-d H:i:s');
	...
	$this->set(compact('abstract_detail','subabstractcount','tot_amount','units','projectDevelopmentWorkDetail', 'projectWorks','buildingItems','projectWorkSubdetails', 'developmentWorks','id','work_id','abstract_subdetails','detailed_approval_stages_count','detailed_approval_stages','abstractcount','projectWorkSubdetail'));
    }
  ...
  public function ajaxitemcode($id = null)
    {
     ...
     public function projectexpenditure($id = null,$work_id = null)   
   {	   
	   $this->viewBuilder()->setLayout('layout');
	   $this->loadModel('ProjectWorkSubdetails');
	   $this->loadModel('ProjectwiseExpenditureDetails');
	   ...
	   $division_id = $user->division_id; 
	    ...
	    $projectWorkSubdetail     = $this->ProjectWorkSubdetails->find('all')->contain(['Divisions','Circles'])->where(['ProjectWorkSubdetails.id' => $work_id])->first();
        if ($this->request->is(['patch', 'post', 'put'])) {  //echo '<pre>';  print_r($this->request->getData()); exit(); 
           if($this->request->getData('expenditure_incurred') >0){  
            $projectwiseExpenditureDetail = $this->ProjectwiseExpenditureDetails->newEmptyEntity();
			$projectwiseExpenditureDetail->project_work_id           = $id;
			$projectwiseExpenditureDetail->project_work_subdetail_id = $work_id;
			$projectwiseExpenditureDetail->month                     = date('Y-m');            
			$projectwiseExpenditureDetail->expenditure               = $this->request->getData('expenditure_incurred');
			$projectwiseExpenditureDetail->created_by                = $user->id;
			$projectwiseExpenditureDetail->created_date              = date('Y-m-d H:i:s');							  
			if($this->ProjectwiseExpenditureDetails->save($projectwiseExpenditureDetail)){
				$subdetailTable                    = $this->getTableLocator()->get('ProjectWorkSubdetails');
				$subproject                        = $subdetailTable->get($projectWorkSubdetail['id']); 
				$subproject->expenditure_incurred  = $this->request->getData('expenditure_incurred');     
				$subdetailTable->save($subproject);		
				
			}
			$this->Flash->success(__('The Expenditure Incurred has been updated Successfully.'));
		   }
			return $this->redirect(['controller' => 'ProjectWorks', 'action' => 'projectlist/18']);
		}		
       $this->set(compact('projectWork','projectwiseDetailedEstimate', 'projectWorks', 'materials', 'units','detailed_estimates','id','administrativesanctioncount','administrativesanction','projectWorkSubdetail','work_id','total_estimate'));
	}
    ...
    public function deletework($id = null)
    {
	 ...
	 public function deleteproposalwork($id = null)
    {
       $this->loadModel('ProjectWorkSubdetails');
	   $projectsubdetail = $this->ProjectWorkSubdetails->find('all')->where(['ProjectWorkSubdetails.project_work_id'=>$id])->first();
		...
		$user->is_active  = 0;
        if ($userTable->save($user)) {
			$this->Flash->success(__('The Project Proposal has been Deleted.'));
