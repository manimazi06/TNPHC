...
class OldProjectWorkDetailsController extends AppController  
{
    public function index()
    {
		...
		if ($this->request->is('post')) {
			...
			$query = "SELECT old.project_name as projectname,old.id as id,
                    ...
                    LEFT JOIN project_work_subdetails ps on ps.old_project_work_detail_id = old.id 
                    LEFT JOIN project_work_statuses ws on ws.id = ps.project_work_status_id
	...
	public function pcrworklist()
    {
		...
		if ($this->request->is('post')) {
			...
			$query = "SELECT old.project_name as projectname,old.id as id,
                    ...
                    LEFT JOIN circles c on c.id = old.circle_id 
                    LEFT JOIN financial_years fy on fy.id = old.financial_year_id 
					where old.is_active=1 and old.work_completed = 0 and old.work_type = 3 $condition $fin_year_cond $dist_cond $ref_no_cond $div_cond  order by fy.id DESC";
        ...
        $query = "SELECT  old.project_name as projectname,old.id as id,
                    ...
                    LEFT JOIN circles c on c.id = old.circle_id 
                    LEFT JOIN financial_years fy on fy.id = old.financial_year_id 
					where old.is_active=1 and old.work_completed = 0 and old.work_type = 3 $condition order by fy.id DESC";
		}
	...
	public function completedprojectlist()
    {
		...
		if ($this->request->is('post')) {
			...
			$query = "SELECT old.project_name as projectname,old.id as id,
                    ...
                    LEFT JOIN financial_years fy on fy.id = old.financial_year_id 
                    LEFT JOIN departments d on d.id = old.department_id 
					where old.is_active=1 and old.work_completed = 1 and old.work_type = 1  $condition $fin_year_cond $dept_cond $go_cond $div_cond  order by fy.id DESC";
        ...
        $query = "SELECT  old.project_name as projectname,old.id as id,
                    ...
                    LEFT JOIN financial_years fy on fy.id = old.financial_year_id 
					LEFT JOIN departments d on d.id = old.department_id 
					where old.is_active=1 and old.work_completed = 1 and old.work_type = 1 $condition order by fy.id DESC";
		}
   ...
   //        echo"<pre>";print_r($id,$pid,$work_id);exit();
	...
	//echo "<pre>"; print_r($oldProjectWorkDetail); exit();
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($attachment->getClientFilename() != '') {
                ...
                $newfile                                   =  "Project_".$id."_". $current_time . "." . $fileExt;
                $tempFile                                  =  $tmpName;
	...
	public function repairbasicdetail($id = null, $pid = null, $work_id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>";  print_r($this->request->getData());  exit();
            ...
            if ($attachment->getClientFilename() != '') {
                ...
                $fileExt                                   =  $array[count($array) - 1];
                $current_time                              =  date('Y_m_d_H_i_s');
                $newfile                                   =  "Repairworks_".$id."_". $current_time . "." . $fileExt;
                $tempFile                                  =  $tmpName;
                $targetPath                                =  'uploads/ProjectWorks/';
                $targetFile                                =  $targetPath . $newfile;
                $projectWork->file_upload                  =  $newfile;
	...
	public function pcrbasicdetail($id = null, $pid = null, $work_id = null)
    {
        $this->viewBuilder()->setLayout('layout');
        $user = $this->request->getAttribute('identity');
        $role_id     = $user->role_id;
        ...
        if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>";  print_r($this->request->getData());  exit();
            ...
            if ($this->ProjectWorks->save($projectWork)) {
				...
				$projectcode               =  'PCR'.$fyear.$depcode.$var;				 
				$ProjectWorksTable      = $this->getTableLocator()->get('ProjectWorks');
				$project                = $ProjectWorksTable->get($insertid); 
				$project->project_code  = $projectcode;
				$ProjectWorksTable->save($project);		
				...
				$projectWorkSubdetail->circle_id               = $this->request->getData('project_circle_id1');
				$projectWorkSubdetail->rough_cost              = $this->request->getData('estimated_cost');
				$projectWorkSubdetail->work_type               = 3;
				$projectWorkSubdetail->project_work_status_id  = 1;
			    $projectWorkSubdetail->old_project_flag        = 1;
                $projectWorkSubdetail->old_project_work_detail_id = $id;
				$projectWorkSubdetail->is_approved             = 1;
   ...
   public function completedbasicdetail($id = null, $pid = null, $work_id = null)
    {
        $this->viewBuilder()->setLayout('layout');
        $user = $this->request->getAttribute('identity');
        $role_id     = $user->role_id;
        ...
        if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>";  print_r($this->request->getData());  exit();
            ...
            if ($this->ProjectWorks->save($projectWork)) {				
				...
				$projectcode               =  'P'.$fyear.$depcode.$var;				 
				$ProjectWorksTable      = $this->getTableLocator()->get('ProjectWorks');
				$project                = $ProjectWorksTable->get($insertid); 
				$project->project_code  = $projectcode;
				$ProjectWorksTable->save($project);		
				...
				$projectWorkSubdetail->circle_id               = $this->request->getData('project_circle_id1');
				$projectWorkSubdetail->rough_cost              = $this->request->getData('estimated_cost');
				$projectWorkSubdetail->work_type               = 1;
				$projectWorkSubdetail->is_work_completed       = 1;
				$projectWorkSubdetail->completion_date         = date('Y-m-d',strtotime($this->request->getData('completion_date')));
				$projectWorkSubdetail->project_work_status_id  = 19;
			    $projectWorkSubdetail->old_project_flag        = 1;
                $projectWorkSubdetail->old_project_work_detail_id = $id;
				$projectWorkSubdetail->is_approved             = 1;
                $projectWorkSubdetail->submit_date             = date('Y-m-d');
               ...
               // echo"<pre>";print_r($projectWorkSubdetail);exit();
                if ($this->ProjectWorkSubdetails->save($projectWorkSubdetail)) {
                    $sub_insert_id = $projectWorkSubdetail->id;				
					        ...
					        $projectFinancialSanction->project_work_id         =  $insertid;
							...
							$attachment1               =   $this->request->getData('sanctioned_file_upload');
							if ($attachment1->getClientFilename() != '') {
								...
								if ($name1 != '' && $error1 == 0) {
									...
									$newfile1                                   =  "financial_sanction_" . $current_time1 . "." . $fileExt1;
									$tempFile1                                  =  $tmpName1;
									$targetPath1                                =  'uploads/financialsanction/';
									$targetFile1                                =  $targetPath1.$newfile1;
									$projectFinancialSanction->sanctioned_file_upload                  =   $newfile1;
					   ...
					   $this->Flash->success(__('The completed project work has been saved.'));
					   return $this->redirect(['action' => 'completedprojectlist']);
                }                
            }
        }
        $this->loadModel('FinancialYears');
	   ...
	   if($projectWork['department_id'] != ''){       
        ...
        $this->set(compact(
            ...
            'schemeTypes',
            'work_id',
            'oldProjectWorkDetail','projectFinancialSanction'
        ));
    }	
    ...
    public function administrativesanction($id, $pid, $work_id)
    {
        ...
        $projectworksubdetails  = $this->ProjectWorkSubdetails->find('all')->where(['ProjectWorkSubdetails.id' => $work_id, 'ProjectWorkSubdetails.is_active' => 1])->first();
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($attachment->getClientFilename() != '') {
                ...
                $fileExt                                   =  $array[count($array) - 1];
                $current_time                              =  date('Y_m_d_H_i_s');
                $newfile                                   =  "administativesanction_".$pid."_" . $current_time . "." . $fileExt;    
                $tempFile                                  =  $tmpName;
           ...
           if ($administrativeSanction['id'] != '') {
            ...
            if ($this->ProjectAdministrativeSanctions->save($projectAdministrativeSanction)) {
				...
				if ($administrativeSanction['id'] == '' && $projectworksubdetails['work_type'] == 1) {
    ...
    public function projectdetailedestimate($id = null, $pid = null, $work_id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($attachment->getClientFilename() != '') {
                $name1    = $attachment->getClientFilename();
                $type    = $attachment->getClientMediaType();
                $size    = $attachment->getSize();
                $tmpName = $attachment->getStream()->getMetadata('uri');
                $error   = $attachment->getError();
                ...
                $fileExt                                  = $array[count($array) - 1];
                $current_time                             = date('Y_m_d_H_i_s');
                $newfile                                  = "detailed_estimate_".$work_id."_". $current_time . "." . $fileExt;
                $tempFile                                 = $tmpName;
   ...
   // $this->loadModel('Notifications');
    ...
    $division_id = $user->division_id;
     ...
     $detailed_approval_stages        = $this->DetailedEstimateApprovalStages->find('all')->contain(['ApprovalStatuses'])->where(['DetailedEstimateApprovalStages.project_work_id' => $pid,'DetailedEstimateApprovalStages.project_work_subdetail_id'=>$work_id])->toArray();
	  ...
	  // print_r($Totalamount); exit();
	   ...
	   $technical       = $this->TechnicalSanctions->find('all')->where(['TechnicalSanctions.project_work_id' => $pid, 'TechnicalSanctions.project_work_subdetail_id' => $work_id])->first();
	   $additional_count   = $this->ProjectwiseAbstractAdditionalDetails->find('all')->where(['ProjectwiseAbstractAdditionalDetails.project_work_id' => $pid, 'ProjectwiseAbstractAdditionalDetails.project_work_subdetail_id' => $work_id,'ProjectwiseAbstractAdditionalDetails.is_active'=>1])->count();
	   $additional_details = $this->ProjectwiseAbstractAdditionalDetails->find('all')->where(['ProjectwiseAbstractAdditionalDetails.project_work_id' => $pid, 'ProjectwiseAbstractAdditionalDetails.project_work_subdetail_id' => $work_id,'ProjectwiseAbstractAdditionalDetails.is_active'=>1])->toArray();
    ...
    if ($this->request->is(['patch', 'post', 'put'])) {
			  ...
			  if($this->request->getData('grand_total') != ''){
			   ...
			   if(count($norms) > 0){ 
					foreach($norms as $norm){					            
					 ...
					 $additional->project_work_id           = $pid;
        ...
        if ($this->ProjectwiseAbstractDetails->save($abstractdetail)) { 
            ...
            foreach ($this->request->getData('workdetail') as $key => $answer) {
				...
				// }
				$abstractsubdetail->new_item_description    = ($answer['item_description'] != '')?$answer['item_description']:$answer['item_description1'];
				...
				if($abstractsubdetail->new_item_description != ''){
	...
	$this->set(compact('additional_count','additional_details','abstract_detail','oldProjectWorkDetail','projectwork','technical','work_type','subabstractcount','tot_amount','units','projectDevelopmentWorkDetail', 'projectWorks','buildingItems','projectWorkSubdetails', 'developmentWorks','id','pid','work_id','abstract_subdetails','detailed_approval_stages_count','detailed_approval_stages','abstractcount','projectWorkSubdetail'));
   ...
   $abstract_detail = $this->ProjectwiseAbstractDetails->find('all')->where(['ProjectwiseAbstractDetails.project_work_id'=>$pid,'ProjectwiseAbstractDetails.project_work_subdetail_id'=>$work_id])->first();
	...
	if($abstract_detail['gst_at_18'] == ''){
    $total_amount = $technical['amount']+$technical['gst'];
    }else{
	$total_amount = $abstract_detail['sub_total_2'];
	}
	...
	public function contractorratedetails($id = null, $pid = null, $work_id = null)
    {
		...
		//print_r($contractor_detailcount); exit();
	...
	public function newabstractdetail($id = null, $pid = null, $work_id = null)
    {
        $this->viewBuilder()->setLayout('layout');
        $this->loadModel('ProjectWorks');
        $this->loadModel('ContractorDetails');
	 ...
	 $abstract_subdetails = $this->ProjectwiseAbstractSubdetails->find('all')->contain(['BuildingItems','Units'])->where(['ProjectwiseAbstractSubdetails.projectwise_abstract_detail_id'=>$abstract_detail['id'],'ProjectwiseAbstractSubdetails.is_active'=>1])->order('ProjectwiseAbstractSubdetails.id ASC')->toArray();
        ...
        if ($this->request->is(['patch', 'post', 'put'])) { //echo"<pre>"; print_r($this->request->getData());  exit();
				 ...
				 foreach ($this->request->getData('workdetail') as $key => $answer) {					
						 $ProjectWorksTable          = $this->getTableLocator()->get('ProjectwiseAbstractSubdetails');
						 $project                    = $ProjectWorksTable->get($answer['id']); 
						 $project->new_building_item_id   = $answer['building_item_id'];
						 $project->new_item_code          = $answer['item_code'];
						 $project->new_item_description   = $answer['item_description'];
						 $ProjectWorksTable->save($project);				
 				   }
				   return $this->redirect(['action' => 'newabstractdetail/' . $id . '/' . $pid . '/' . $work_id]);
	  ...
	  $buildingItems     = $this->NewBuildingItems->find('list',  ['keyField' => 'id', 'valueField' => 'item_code'])->order(['NewBuildingItems.building_item_type_id ASC'])->toArray();
        ...
        $this->set(compact(
            ...
            'technical',
            'pid',
            'tender_amount','work_type','contractor_detailcount','contractoratedetails','contractorcount','project','buildingItems'
        ));
    }

   public function abstractdetail($id = null, $pid = null, $work_id = null)
    {
        $this->viewBuilder()->setLayout('layout');
        $this->loadModel('ProjectWorks');
        $this->loadModel('ContractorDetails');
    ...
    if ($this->request->is(['patch', 'post', 'put'])) {  //echo"<pre>"; print_r($this->request->getData());  exit();
       ...
       if ($totalunit != '') {
        ...
        if ($this->request->getData('is_permission_apporved') == 1) {
            ...
            if ($attachment->getClientFilename() != '') {
                ...
                if ($name != '' && $error == 0) {
                    ...
                    $fileExt                                  = $array[count($array) - 1];
                    $current_time                             = date('Y_m_d_H_i_s');
                    $newfile                                  = "PlanningPermission_".$work_id."_" . $current_time . "." . $fileExt;
                    $tempFile                                 = $tmpName;
                    $targetPath                               = 'uploads/PlanningPermissions/';
                    $targetFile                               = $targetPath . $newfile;
        ...
        if ($this->PlanningPermissionDetails->save($planningPermissionDetail)) {
           //$this->Flash->success(__('The planning permission detail has been saved.'));
            if ($this->request->getData('is_permission_apporved') == 1) {
				if ($projectWorkSubdetail['planning_permission_flag'] == 0) {
                $subdetailTable                 = $this->getTableLocator()->get('ProjectWorkSubdetails');
                $project                        = $subdetailTable->get($work_id);
                $project->planning_permission_flag     = 1;
                $project->project_work_status_id       = 10;
				...
				//echo '<pre>';  print_r($attachment); exit();
				 ...
				 if ($attachment->getClientFilename() != '') {  
					 ...
					 if ($name != '' && $error == 0) {
						 ...
						 $fileExt                                  = $array[count($array) - 1];
						 $current_time                             = date('Y_m_d_H_i_s');
						 $newfile                                  = "drawing_".$work_id."_" . $current_time . "." . $fileExt;
						 $tempFile                                 = $tmpName;
   ...
   public function repairworklist()
   {
	...
	$role_id     = $user->role_id;
	 ...
	 $abstract_subdetails = $this->ProjectwiseAbstractSubdetails->find('all')->contain(['BuildingItems','Units'])->where(['ProjectwiseAbstractSubdetails.id'=>$abstract_id,'ProjectwiseAbstractSubdetails.is_active'=>1])->toArray();
    ...
    //}
     ...
     // }
	  ...
	  $newbuildingItems     = $this->NewBuildingItems->find('list',  ['keyField' => 'id', 'valueField' => 'item_code'])->all();


	$this->set(compact('newbuildingItems','units','projectDevelopmentWorkDetail', 'projectWorks','buildingItems','projectWorkSubdetails', 'developmentWorks','id','work_id','abstract_subdetails'));
    }
	...
	public function ajaxaddnorm($i = null){
		
		 $this->set(compact('i'));
	}	
	
    public function deletenorm($id = null){
      if($id != ''){		   
           $workTable       = $this->getTableLocator()->get('ProjectwiseAbstractAdditionalDetails');
		   $work            = $workTable->get($id); 
		   $work->is_active = 0;
