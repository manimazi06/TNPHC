...
class OpeningBalanceLogsController extends AppController
{
    ...
    {
        $this->loadModel('OpeningBalanceDetails');
        $this->loadModel('FundRequestUserDepartmentDetails');
        $this->loadModel('FundRequestUserDepartmentSubdetails');
        $this->viewBuilder()->setLayout('layout');
        $user = $this->request->getAttribute('identity');
        $div_id = $user->division_id;
        $role_id = $user->role_id;		
        ...
        $query      =  "SELECT psd.work_name,dv.name as division_name,fs.go_no as fsgo_no,sum(frd.request_amount) as request_amount,psd.project_work_id,psd.id as work_id
						 ...
						 group by psd.work_name,dv.name,fs.go_no,psd.project_work_id,psd.id
						 ";										 
    ...
    public function funddetailsedit($id = null)
    {
       ...
       // $amount_received    = $this->OpeningBalanceLogs->IsAmountReceives->find('list', ['keyField' => 'id', 'valueField' => 'name'])->all();
        ...
        $this->set(compact('openingBalanceLog', 'amount_received', 'role_id'));
    }
