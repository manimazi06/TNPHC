...
class TenderStatusesController extends AppController
{
    ...
    public function delete($id = null)
    {
        ...
        $tenderStatus            = $this->TenderStatuses->get($id);
        $tenderStatus->is_active =0;
            // echo"<pre>";print_r($tenderStatus);exit();
            if ($this->TenderStatuses->save($tenderStatus)) {
                $this->Flash->success(__('The tenderStatus has been deleted.'));
            } else {
                $this->Flash->error(__('The tenderStatus could not be deleted. Please, try again.'));
            }
