   ...
   public function logout()
    ...
    public function index()
    {
		 ...
		 if ($this->request->is('post')) {
			...
			$users = $this->Users->find('all')->contain(['Roles'])->where(['Users.role_id'=>$roleid,'Users.is_active'=>1])->toArray();
		 }else{
			$users = $this->Users->find('all')->contain(['Roles'])->where(['Users.is_active'=>1])->toArray();
		 }
  ...
  public function view($id = null)
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($username > 0) {
                $this->Flash->error(__('User Name is already exits'));
                // return $this->redirect(['action' => 'index']);
            } elseif ($mobile_no > 0) {
                $this->Flash->error(__('mobile no is already exits'));
            } elseif ($email) {
