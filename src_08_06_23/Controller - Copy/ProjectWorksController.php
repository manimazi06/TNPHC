...
{		
   ...
   public function dashboard()
	...
	where project.is_active=1 and project.project_work_status_id>=1 $condition";
		...
		$this->set(compact('yearwise_detail', 'financialYears', 'departwise_as_details', 'progressCount', 'Totalcompletecount', 'TotalProjectCount', 'department_details', 'divisions_details', 'role_id', 'division_id', 'projectProposedCount', 'projectApprovedCount', 'projectSanctionedcount', 'projectProgresscount', 'projectCompletedcount'));
	}
	...
	public function ajaxprojectwise($val = NULL)
    {
        ...
        $sql = "SELECT  department.name as dname,
                    ...
                    LEFT JOIN divisions as division on division.id = work_subdetails.division_id
                    LEFT JOIN project_work_statuses as project_work_statuse on project_work_statuse.id = work_subdetails.project_work_status_id
                    where work_subdetails.is_active=1 $Cond";
...
//echo '<pre>';  print_r($projectWorkSubdetails);   exit();
    ...
    public function projectapprove($id = null)
   {
	...
	public function administrativesanctionadd(){  
		...
		if ($this->request->is(['patch', 'post', 'put'])) { 
		  ...
		  // echo "<pre>";  print_r($approved_project_count); exit();
   ...
   {	   
	 ...
	 public function tenderdetails($id = null, $work_id = null)
    {
        ...
        $tender_count = $this->ProjectTenderDetails->find('all')->where(['ProjectTenderDetails.project_work_id' => $id, 'ProjectTenderDetails.project_work_subdetail_id' => $work_id])->count();
       ...
       if($project_tender_statusescount == 0){
		$tenderStatus = $this->TenderStatuses->find('list')->toArray();  
         ...
         $tenderStatus = $this->TenderStatuses->find('list',  ['keyField' => 'id', 'valueField' => 'name'])->where(['TenderStatuses.id NOT IN'=>$entered_id])->all();  
	   }
	   ...
	   //echo '<pre>'; print_r($tenders); exit();
        ...
        if ($this->request->is(['patch', 'post', 'put'])) { //echo '<pre>'; print_r($this->request->getData());  exit();
			...
			if($tender_status != 4  && $completed_flag == ''){
        ...
        $this->set(compact('tender_count','project_tender_statusescount','project_tender_statuses','tenderStatus','projectTenderDetail', 'tender_type', 'projectWork', 'tenders', 'id', 'administrativesanctioncount', 'administrativesanction', 'projectWorkSubdetail', 'financialSanctionscount', 'financialSanctions', 'work_id', 'contractor_detail_count', 'contractor_details', 'technicalcount', 'technical'));
    }
    ...
    public function addcontractor($id = null, $work_id = null, $tender = null)
    {
	 ...
	 if($abstractcount > 0){
	 $abstract_subdetails = $this->ProjectwiseAbstractSubdetails->find('all')->contain(['BuildingItems','Units'])->where(['ProjectwiseAbstractSubdetails.projectwise_abstract_detail_id'=>$abstract_detail['id']])->order('ProjectwiseAbstractSubdetails.id ASC')->toArray();
	  } 
	...
	public function sitehandover($id = null,$work_id = null)
    {
       ...
       $contractor_details          = $this->ContractorDetails->find('all')->contain(['ProjectWorks'])->where(['ContractorDetails.project_work_id' => $id,'ContractorDetails.project_work_subdetail_id' => $work_id])->first(); //tender key
	    ...
	    //$date = $contractor_details['agreement_fromdate'];
	    // $date = $contractor_details['agreement_fromdate'];
      ...
      // print_r($date);
		...
		// $date1 = strtotime(date('Y-m-d',strtotime($contractor_details['agreement_fromdate'])));
		
		
		// $date2 = strtotime(date('Y-m-d',strtotime($contractor_details['agreement_todate'])));
  ...
  // echo '<pre>'; print_r($requestcount); exit();
			...
			if ($this->request->is(['patch', 'post', 'put'])) {
				...
				if ($attachment1->getClientFilename() != '') {
					...
					if ($name1 != '' && $error1 == 0) {
						...
						$newfile1                                  = "drawing_copy_" . $current_time1 . "." . $fileExt;
						$tempFile1                                 = $tmpName1;
    ...
    public function funddetails($id = null, $work_id = null)
   {
		  ...
		  if ($this->request->is(['patch', 'post', 'put'])) {			  
			  $request_month = $this->request->getData('request_month');	  
			   ...
			   if($role_id == 6 || $role_id == 8 || $role_id == 16){	
				 ...
				 $division =  $this->Divisions->find('list',['keyField' => 'id', 'valueField' => 'id'])->where(['Divisions.circle_id'=>$circle_id])->toArray();
		   ...
		   $technicalSanction = $this->TechnicalSanctions->newEmptyEntity();		     
			$attachment  = $this->request->getData('detailed_estimate_upload');
             ...
             if($this->TechnicalSanctions->save($technicalSanction)){
				...
				/*if($projectWorkSubdetail['approval_role'] == 4){
				$detailedEstimateApprovalStage->current_status            = 'Forwarded to EE';  
				}else if($projectWorkSubdetail['approval_role'] == 5){
				$detailedEstimateApprovalStage->current_status            = 'Forwarded to SE';  
				}else if($projectWorkSubdetail['approval_role'] == 6){
	...
	$this->set(compact('tot_amount','units','projectDevelopmentWorkDetail', 'projectWorks','buildingItems','projectWorkSubdetails', 'developmentWorks','id','work_id','abstract_subdetails','detailed_approval_stages_count','detailed_approval_stages','abstractcount','projectWorkSubdetail'));
    }
